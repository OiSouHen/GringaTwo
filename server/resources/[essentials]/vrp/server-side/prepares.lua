vRP.prepare("vRP/get_infos","SELECT * FROM vrp_infos WHERE steam = @steam")
vRP.prepare("vRP/get_characters","SELECT id,registration,phone,name,name2,loc FROM vrp_users WHERE steam = @steam and deleted = 0")
-----------------------------------------------------------------------------------------------------------------------------------------
-- PREPARE RACES
-----------------------------------------------------------------------------------------------------------------------------------------
...
-- PREPARE USERS
-----------------------------------------------------------------------------------------------------------------------------------------
vRP.prepare("vRP/get_users","SELECT * FROM vrp_users WHERE id = @id")
vRP.prepare("vRP/get_registration","SELECT id FROM vrp_users WHERE registration = @registration")
vRP.prepare("vRP/get_phone","SELECT id FROM vrp_users WHERE phone = @phone")
vRP.prepare("vRP/create_characters","INSERT INTO vrp_users(steam,name,name2,loc) VALUES(@steam,@name,@name2,@loc)")
vRP.prepare("vRP/update_nationality","UPDATE vrp_users SET loc = 'Norte' WHERE loc = 'Sul' AND id = @id")
...
vRP.prepare("vRP/update_premium","UPDATE vrp_infos SET predays = predays + @predays, priority = @priority WHERE steam = @steam")
-----------------------------------------------------------------------------------------------------------------------------------------
-- PREPARE HOMES
-----------------------------------------------------------------------------------------------------------------------------------------
...
vRP.prepare("vRP/update_twogarages","UPDATE vrp_users SET garage = garage + 2 WHERE id = @id")
...
-- PREPARE GEMS
-----------------------------------------------------------------------------------------------------------------------------------------
vRP.prepare("vRP/set_gemsitem","UPDATE vrp_infos SET gems = gems + 1 WHERE steam = @steam")
vRP.prepare("vRP/set_gems","UPDATE vrp_infos SET gems = gems + @gems WHERE steam = @steam")
vRP.prepare("vRP/rem_gems","UPDATE vrp_infos SET gems = gems - @gems WHERE steam = @steam")
