...
-- THREADSEARCHCOINS
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("garbageman:searchCoins")
AddEventHandler("garbageman:searchCoins",function(searchCoins)
    local idle = 1000
    local ped = GetPlayerPed(-1)
    local pedCoords = GetEntityCoords(ped, 0)
    local objectEmpty = false
    
    for k,v in pairs(coinsCans) do
        local object = GetClosestObjectOfType(pedCoords["x"], pedCoords["y"], pedCoords["z"], 1.0, GetHashKey(v[1]), true, true, true)
        SetEntityAsMissionEntity(object, true, true)
        if DoesEntityExist(object) then
            coinsCoords = GetEntityCoords(object, 0)
        end
    end
    
    if coinsCoords ~= nil then
        local distance = GetDistanceBetweenCoords(pedCoords["x"], pedCoords["y"], pedCoords["z"], coinsCoords["x"], coinsCoords["y"], coinsCoords["z"])
        if distance < 1.5 then
            idle = 5   
        end
    ...
    if coinsCoords ~= nil then
        if (GetDistanceBetweenCoords(pedCoords["x"], pedCoords["y"], pedCoords["z"], coinsCoords["x"], coinsCoords["y"], coinsCoords["z"] < 1)) and (not IsPedInAnyVehicle(ped)) then
            
            if not IsPauseMenuActive() and not exports["inventory"]:blockInvents() and not exports["player"]:blockCommands() and not exports["player"]:handCuff() and GetEntityHealth(ped) > 101 and not IsEntityInWater(ped) then
                if (GetDistanceBetweenCoords(pedCoords["x"], pedCoords["y"], pedCoords["z"], coinsCoords["x"], coinsCoords["y"], coinsCoords["z"] < 0.5)) then
                    TriggerEvent("cancelando",true)
					vRP.playAnim(false,{"amb@prop_human_parking_meter@female@idle_a","idle_a_female"},true)
                    TriggerEvent("Progress",30000,"Vasculhando...")
                    Wait(30000)
                    if vSERVER.searchCoins(coinsCoords["x"]) then
                    end
                    TriggerEvent("cancelando",false)
                    ClearPedTasks(ped)
                end
