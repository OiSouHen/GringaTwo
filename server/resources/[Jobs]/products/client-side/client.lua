		...
		while true do
			if timeSelling > 0 then
				...
				if timeSelling <= 0 then
					...
					if (lastItem == "joint" or lastItem == "joint" or lastItem == "ecstasy" or lastItem == "lean" or lastItem == "meth") and math.random(100) >= 90 then
						GiveWeaponToPed(inPed,GetHashKey("WEAPON_MICROSMG"),250,true,true)
						TaskShootAtEntity(inPed,ped,25000,GetHashKey("FIRING_PATTERN_FULL_AUTO"))
						
						SetTimeout(25000,function()
							ClearPedTasks(inPed)
							TaskWanderStandard(inPed,10.0,10)
							TaskReactAndFleePed(inPed,ped)
							SetPedKeepTask(inPed,true)
						end)
					end
				end
