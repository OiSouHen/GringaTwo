...
function init() {
	...
	websocket.onmessage = (evt) => {
		...
		if (evt.data == 'startedtalking'){
			$.post('http://tokovoip/setPlayerTalking',JSON.stringify({ state: 1 }));
		}

		if (evt.data == 'stoppedtalking'){
			$.post('http://tokovoip/setPlayerTalking',JSON.stringify({ state: 0 }));
		}
	};
...
function updateScriptData(key,data){
	...
	$.post(`http://tokovoip/updatePluginData`,JSON.stringify({ payload: { key,data }	}));
}
