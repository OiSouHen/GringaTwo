...
$(document).ready(function(){
	window.addEventListener("message",function(event){
		...
		if (event["data"]["radar"] == "top"){
			$("#topRadar").html("<c>PLACA:</c> "+ event["data"]["plate"] +"     <c>MODELO:</c> "+ event["data"]["model"] +"     <c>VELOCIDADE:</c> "+ parseInt(event["data"]["speed"]) +" MPH");
		}

		if (event["data"]["radar"] == "bot"){
			$("#botRadar").html("<c>PLACA:</c> "+ event["data"]["plate"] +"     <c>MODELO:</c> "+ event["data"]["model"] +"     <c>VELOCIDADE:</c> "+ parseInt(event["data"]["speed"]) +" MPH");
		}
	});
