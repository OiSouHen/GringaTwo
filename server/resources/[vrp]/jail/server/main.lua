...
-----------------------------------------------------------------------------------------------------------------------------------------
CNVcore = {}
Tunnel.bindInterface("jail",CNVcore)
vCLIENT = Tunnel.getInterface("jail")
...
function CNVcore.saveItems()
    ...
    if user_id then
		...
		TriggerClientEvent("Notify",source,"amarelo","Você poderá retirar seus pertences ná recepção assim que for liberado.",5000)
	end
end
