...
local function DriveInGarage()
	...
	if DoesEntityExist(veh) then
		...
		LSCMenu:addPurchase("Reparar",5500,nil)
...
--									DrawText3D(outside.x,outside.y,outside.z,"MENTALIZE ~g~ENTER~w~ PARA ENTRAR")
								end
							else
								DrawText3D(outside.x,outside.y,outside.z,"~r~OCUPADO ATUALMENTE~w~")
							end
						end
					end
				end
