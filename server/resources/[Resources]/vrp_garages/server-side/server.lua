    ...
    if user_id and name then
        if not vCLIENT.returnVehicle(source,name) then
			...
			elseif vRP.vehicleType(tostring(name)) == "donate" and vRP.getCarPremium(name,user_id)then
				local status = vRP.request(source,"Este veículo está atrasado, deseja renovar este veículo pagando <b>$"..vRP.format(parseInt(vRP.vehiclePrice(name))).." Coins</b>?",60)
				if status then
	...
	if user_id then
		...
		if vehicle and vehPlate then
			...
			if user_id == plateUserId or stealVehs[vehPlate] == user_id then
				...
				if vehLock == 1 then
					TriggerClientEvent("Notify",source,"car-lock-open","<div style='opacity: 0.7;'><i>Mensagem de Segurança</i></div>Você acabou de <b>abrir</b> as portas do seu carro.",5000)
					TriggerClientEvent("vrp_sound:source",source,"unlock",0.5)
				else
					TriggerClientEvent("Notify",source,"car-lock","<div style='opacity: 0.7;'><i>Mensagem de Segurança</i></div>Você acabou de <b>trancar</b> as portas do seu carro.",5000)
